...
import { CLI_ARGS_OBJ, PROJECT_ROOT, getCustomConfig, type ExternalOption } from './base';
import { defineConfig, esmExternalRequirePlugin, type LibraryFormats } from 'vite';
import eslint from 'vite-plugin-eslint2';

const REACT_EXTERNAL_PATTERN = /^react(-dom)?(\/.+)?$/;

const matchesExternal = (external: ExternalOption, id: string, importer?: string, isResolved?: boolean) => {
  ...
  return external.some((externalId) => {
    ...
    externalId.lastIndex = 0;
    return externalId.test(id);
  });
};

const withReactExternalPolicy = (
  external: ExternalOption,
  isReactExternal: (id: string) => boolean
): ExternalOption => {
  return (id, importer, isResolved) => {
    if (REACT_EXTERNAL_PATTERN.test(id)) {
      return isReactExternal(id);
    }

    return matchesExternal(external, id, importer, isResolved);
  };
};
...
export const getCommonConfig = async (specifiedFormats?: LibraryFormats[]) => {
  ...
  const isAllBrowserFormats = formats.every((format) => format === 'umd' || format === 'iife');
  const useEsmExternalRequirePlugin = !isAllBrowserFormats;
  const reactExternalRequirePlugin = esmExternalRequirePlugin({
    external: [REACT_EXTERNAL_PATTERN],
  });
  const resolveExternal = (external: ExternalOption) => {
    return useEsmExternalRequirePlugin ? withReactExternalPolicy(external, () => false) : external;
  };
  ...
  const getExternal = () => {
    ...
    if (CUSTOM_CONFIG.externalByFormat) {
      ...
      if (formats.length === 1) {
        ...
        if (externalForFormat !== undefined) {
          return resolveExternal(externalForFormat);
        }
      }
      ...
      if (isAllModuleFormats) {
        ...
        if (moduleExternal) {
          return resolveExternal(moduleExternal);
        }
      }
      ...
      if (isAllBrowserFormats) {
        ...
        if (browserExternal) {
          return resolveExternal(browserExternal);
        }
      }
    }
    ...
    if (CUSTOM_CONFIG.external) {
      return resolveExternal(CUSTOM_CONFIG.external);
    }
    ...
    return resolveExternal((id: string) => {
      // 相对路径或绝对路径：项目内部文件
  ...
  if (CUSTOM_CONFIG.libMode === false) {
    ...
    delete commonConfigJson?.build?.rolldownOptions;
  }
