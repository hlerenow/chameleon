---
title: 添加自定义组件物料
description: 同时打通业务组件的物料 Meta、iframe 资源与 Render 运行时映射。
sidebar:
  order: 3
---
...
接入分为两条链路：编辑器 iframe 需要 UMD JS/CSS 和物料描述；发布/预览页可以直接 import React 组件后交给 `Render`。两条链路的组件名和物料协议必须一致。
...
- 组件能从物料库拖入画布，并显示正确样式。
- 物料 `props` 能修改对应 React props。
- 保存后重新加载，节点 `componentName` 不变。
- 同一份保存的 Schema 可在 `Render` 预览页正常显示。
...
- `material[].componentName` 与资源实际导出的组件名一致。
- `assetPackagesList[].package` 与物料中的 npm 包信息一致。
- 编辑器资源可从 iframe 访问，且 JS/CSS 顺序和跨域策略正确。
- 发布页使用 `Render` 时传入对应的 `components` 映射。
- 先在 `/preview` 或独立预览页验证，再接入业务保存和发布流程。
