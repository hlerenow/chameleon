...
1. 先阅读 [Engine 介绍](../reference/Engine/introduction) 和 [Engine 架构](../reference/Engine/architecture)，理解 Engine、Model、Render 的边界。
2. 按本文完成最小接入，并配置 `renderJSUrl`。
3. 使用 [编辑器基础操作](./editor-basics) 设置元素宽高和样式。
4. 需要定位问题时查看 [调试指南](./debugging)，需要独立渲染页面时查看 [预览与渲染](./preview-rendering)。
5. 最后阅读 [自定义组件接入](./addCustomComponent)、物料和插件开发文档。
