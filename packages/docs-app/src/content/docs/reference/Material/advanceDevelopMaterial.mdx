---
title: 高级物料开发
description: 使用 advanceCustom 定制复杂组件的编辑体验。
sidebar:
  order: 3
---
...
高级物料用于常规 `snippets`、`props`、`isContainer` 无法覆盖的场景，例如自定义右侧面板、拖拽行为、DOM 获取、权限控制和快速添加。

## 使用边界

先完成[物料开发](./developMaterial)的基础三件套：React 组件、物料 Meta、UMD JS/CSS。只有需要改变编辑体验时才添加 `advanceCustom`；运行时组件 API 不应依赖编辑器专用字段。
...
- `enableNodeSizeChange`：控制组件是否显示编辑器节点尺寸调整控件，默认不启用。可显式设置为 `true`，或设置为接收当前节点并返回布尔值的函数；节点的 `style` 或 `css` 设置 `display: inline` 或 `display: none` 时不支持。
...
- [物料开发基础文档](./developMaterial)
- [物料协议文档](../PageSchema/material)
- [插件开发指南](../Plugin/plugin-develop)
