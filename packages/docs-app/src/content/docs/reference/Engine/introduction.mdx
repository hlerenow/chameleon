---
title: Engine 介绍
description: 何时使用可视化编辑器，以及如何进入最小接入流程。
sidebar:
  order: 1
---
...
`@chamn/engine` 是基于 React 的可视化页面编辑器容器。它组合 `@chamn/model` 的页面模型、`@chamn/render` 的渲染能力和 `@chamn/layout` 的工作台布局，通过插件提供设计器、组件库、属性面板、历史记录、快捷键等功能。

本页回答“Engine 是什么”；接入代码见[快速开始](../../guides/)，公开 Props 和实例方法见 [Engine API](./api)，模块职责见 [Engine 架构](./architecture)。

## 何时使用 Engine

使用 Engine：需要让运营、设计或业务人员可视化编辑页面，并保存可复用 Schema。

不要在最终用户页面直接使用 Engine：发布页/只读页应该使用 `@chamn/render`，避免引入编辑器工作台和 iframe。
...
| 包              | 责任                     | 在 Engine 中的用途                |
| --------------- | ------------------------ | --------------------------------- |
| `@chamn/engine` | 编辑器容器、插件和工作台 | 提供 `Engine`、内置插件和接入 API |
| `@chamn/model`  | 页面协议、节点和物料模型 | 保存、读取、更新页面节点          |
| `@chamn/render` | 设计态/运行态渲染        | 在画布 iframe 或预览页中渲染页面  |
| `@chamn/layout` | 工作台布局能力           | 承载顶部栏、左右面板和画布        |

`@chamn/engine` 的运行时依赖通过 workspace 包连接。业务侧通常直接安装 Engine、Model、Render，并按需提供物料资源。
...
完成首次挂载后，按以下顺序继续：配置 `renderJSUrl` → 接入业务物料 → 保存 `pageModel.export()` → 使用 Render 验证发布页。每一步都有对应文档：[快速开始](../../guides/)、[添加自定义组件物料](../../guides/addCustomComponent)、[预览与渲染](../../guides/preview-rendering)。
