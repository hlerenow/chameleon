---
title: Engine API
description: Engine 初始化、实例、Workbench 与插件管理 API。
sidebar:
  order: 2
---
...
- `EngineProps` 控制初始化；`workbenchConfig` 仅初始化时生效。
- 页面内容变更优先使用 `engine.updatePage()`，不要直接改外部 `schema` 对象。
- `onReady` 后再调用依赖插件的 API；保存数据使用 `engine.pageModel.export()`。
- 工作台替换、插件增删属于高级定制，先使用默认插件列表验证需求。
    ...
    workbench?.replaceTopBarView(
      <div style={{ padding: '0 20px', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <div>
          ...
          <Button onClick={() => engine.exitPreview()}>退出预览</Button>
