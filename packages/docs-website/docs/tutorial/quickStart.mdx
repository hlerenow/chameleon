...
因为渲染画布是在 iframe 中运行，所以需使用 umd 模式的 js，然后让 iframe 加载，这里只需要将 `render.umd.js` 拷贝到公共资源目录下即可，一般是 `publish` 文件
...
<img style={{ marginBottom: '20px', border: '1px solid #ededed' }} src={engImg} alt="Engine thumbail" />
